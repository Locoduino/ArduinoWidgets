...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
\subsection{How to use the following examples \texttt{ArduinoWidgets}}

~\\In the following sections of this document, there is two pieces of Arduino code that will not be repeated :
~\\The first one is :
\begin{lstlisting}[language=Arduinonl]
#include <ArduinoWidgets.h>
#include <UTFT.h>
...
~\\The second one is in the setup() :
\begin{lstlisting}[language=Arduinonl]
//--- This part is hardware dependent
//--- set up the backlight
  analogWrite (BACKLIGHT, 150);
...
%%%
~\\ HELP !!! j'ai essayé de dessiner un rectangle sans passer par la CustomView précédente mais ...
\begin{lstlisting}[language=Arduinonl]
//--- ca ne marche pas !
  AWRect myRect (420,100, 200,200);
  AWColor myBackColor;
  myBackColor.gray();
  AWColor myFrameColor;
  myFrameColor.red();
  AWView * myRectView;
  myRectView = new AWRectView (myRect, myBackColor, myFrameColor);
  AWRegion * myRegion;
  myRegion = new AWRegion (myRect);
  myRectView->drawInRegion(*myRegion);
  addView (myRectView);
  \end{lstlisting}
