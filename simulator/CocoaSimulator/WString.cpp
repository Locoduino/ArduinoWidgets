...
//--- Added by PM
static void unsignedConversion (const unsigned long long inValue,
                                char * ioString,
                                int & ioIndex,
                                const unsigned inRadix) {
  if (inValue >= inRadix) {
    unsignedConversion (inValue / 10, ioString, ioIndex, inRadix) ;
  }
  const uint8_t v = (uint8_t) (inValue % inRadix) ;
  if (v <= 9) {
    ioString [ioIndex] = (char) (v + '0') ;
  }else{
    ioString [ioIndex] = (char) (v + 'A' - 10) ;
  }
  ioIndex ++ ;
}

static void utoa (const unsigned long inValue, char * inBuffer, const int inRadix) {
  int idx = 0 ;
  unsignedConversion (inValue, inBuffer, idx, (unsigned) inRadix) ;
  inBuffer [idx] = '\0' ;
}

static void ultoa (const unsigned long inValue, char * inBuffer, const int inRadix) {
  utoa (inValue, inBuffer, inRadix) ;
}

static void itoa (const long inValue, char * inBuffer, const int inRadix) {
  int idx = 0 ;
  if (inValue < 0) {
    inBuffer [0] = '-' ;
    idx = 1 ;
    const long v = -inValue ;
    unsignedConversion ((unsigned long) v, inBuffer, idx, (unsigned) inRadix) ;
  }else{
    unsignedConversion ((unsigned long) inValue, inBuffer, idx, (unsigned) inRadix) ;
  }
  inBuffer [idx] = '\0' ;
}

static void ltoa (const long inValue, char * inBuffer, const int inRadix) {
  itoa (inValue, inBuffer, inRadix) ;
}
...
String::String(size_t value, unsigned char base)
{
	init();
	char buf [21];
...
{
	if (cstr) {
		copy(cstr, (unsigned) strlen(cstr));
	} else {
		len = 0;
	}
	return *this;
...
{
	if (cstr) append(cstr, (unsigned) strlen(cstr));
	return *this;
}
...
String & String::append(int num)
{
	...
	append(buf, (unsigned) strlen(buf));
	return *this;
}
...
{
	StringSumHelper &a = const_cast<StringSumHelper&>(lhs);
	if (cstr) a.append(cstr, (unsigned) strlen(cstr));
	return a;
}
...
{
	if (fromIndex >= len) return -1;
//	if (fromIndex >= len || fromIndex < 0) return -1; // Removed comparison always false
...
//  if (s2.len == 0 || len == 0 || s2.len > len || fromIndex < 0) return -1; // Removed comparison always false
...
String & String::replace(const String& find, const String& replace)
{
	...
	int diff = (int) (replace.len - find.len);
	...
	if (diff == 0) {
		...
		int index = (int) (len - 1) ;
		while (index >= 0 && (index = lastIndexOf(find, (unsigned) index)) >= 0) {
			readFrom = buffer + index + find.len;
			memmove(readFrom + diff, readFrom, len - (unsigned) (readFrom - buffer));
			len += (unsigned) diff;
			buffer[len] = 0;
